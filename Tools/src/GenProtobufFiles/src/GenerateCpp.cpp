...
namespace ProtobufGenerate
{
			...
			for (int i = 0 ;i < nMessageCount; ++i)
			{
				...
				if(nMsgID != 0)
				{
					...
					if (strFile.substr(0,2) == MSG_HEADER_FLAG)   //5 只生成CS头的文件
					{
						...
						vtOutputs.push_back(strTabsDefine + "INT32 " + MsgProcessClass + "::Process( " + strMsg + "& msg)");
	...
	BOOL GenerateCPP::GenerateMsgHandlerHeader()
	{
		...
		else
		{
			...
			vtOutputs.push_back("");
			vtOutputs.push_back(strTabsDefine + "public:");
			vtOutputs.push_back(strTabsFunc + "virtual INT32 Process(UINT32 unMsgID, const char* pBuffer, UINT32 unLength);");
			vtOutputs.push_back(strTabsDefine + "public:");
			...
			{ 
				...
				if(nMsgID != 0)
				{
					...
					{  
						...
						vtOutputs.push_back(strTabsFunc + "INT32 Process(" + strMsg + " & msg);" + strNote); 
					} 
				} 
			} 
			vtOutputs.push_back(strTabsDefine + "};"); 
	...
	BOOL GenerateCPP::GenerateMsgHandlerSource()
	{
		...
		else
		{
			...
			vtOutputs.push_back("namespace " + m_strNamespace);
			vtOutputs.push_back("{");
			vtOutputs.push_back(strTabsDefine + "INT32 " + MsgProcessClass + "::Process(UINT32 unMsgID, const char* pBuffer, UINT32 unLength)");
			vtOutputs.push_back(strTabsDefine + "{");
			...
			{ 
				...
				if(nMsgID != 0)
				{
					...
					{  
						...
						vtOutputs.push_back(strTabsCase + "\treturn Process(objProtoMsg);");
						vtOutputs.push_back(strTabsCase + "}");
